...
var linguagem = prompt('Qual a linguagem de programação você está estudando?');
alert(`Olá ${nome}, você tem ${idade} anos e já está aprendendo ${linguagem}!`);
var resposta = 0;
while (resposta != 1 && resposta != 2){
    resposta = Number(prompt(`Você gosta de estudar ${linguagem}? Responda com o número 1 para SIM ou 2 para NÃO.`));
    if (resposta == 1){
        alert('Muito bom! Continue estudando e você terá muito sucesso.')
    } else if (resposta == 2){
        alert('Ahh que pena... Já tentou aprender outras linguagens?')
    } else{
        alert('Você não respondeu com corretamente, responda novamente!')
    }
}